...
class PreviewProcessingSlot 
{
    ...
    public function processFile(FileProcessingService $fileProcessingService, DriverInterface $driver, ProcessedFile $processedFile, File $file, $taskType, array $configuration)
    {
        ...
        if($file->getType() !== File::FILETYPE_VIDEO && !GeneralUtility::inList($GLOBALS['TYPO3_CONF_VARS']['GFX']['videofile_ext'], $file->getExtension()))
        {
            return;
        }
        ...
        if (!file_exists($temporaryFileName)) {
            ...
            $cmd= '/usr/bin/ffmpeg -ss 00:00:01 -i'
                . ' ' . $previewTemporaryFileName
                . ' -frames:v 1'
